...
public class GetEloHistoryUseCase { private final EloHistoryRepository eloHistoryRepository;
    public List<EloHistoryDTO> execute(Long userId) {
        return eloHistoryRepository.findByUserId(userId)
                .stream().map(h -> EloHistoryDTO.builder().id(h.getId())
